...
#define IN4 19
#define ENB 26
#define MODE_PIN 33   // tactile switch for mode toggle (momentary)

// ===== PWM setup =====
const int freq       = 20000; // inaudible
...
const unsigned long commandTimeout = 5000; // 5s failsafe
unsigned long commandStartTime = 0;
const unsigned long maxRunTime = 6000;      // 6s runtime limit

// Mode: 0 = Bluetooth, 1 = Wi-Fi
int mode = 0;
...
// ===== Wi-Fi handler (original HTML intact) =====
void handleHttp() {
  ...
  // full HTML + JS page (unchanged from your original)
...
void setup() {
  ...
  pinMode(MODE_PIN, INPUT_PULLUP);  // stable input for momentary switch

  // PWM setup (ESP32 core v3.3.7 style)
  ledcAttach(ENA, freq, resolution);
  ledcAttach(ENB, freq, resolution);

  stopCar();
  motorSpeed = 0; // ensure safe startup

  WiFi.begin(ssid, password);
  server.begin();

  mode = 0; // force default Bluetooth mode at startup
...
void loop() {
  ...
  if (millis() - lastModePrint > 2000) {
    Serial.println(mode == 0 ? "Mode: Bluetooth" : "Mode: Wi-Fi");
